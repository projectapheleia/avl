...
### Added
 - Mutation testing: `avl-mutation-testing` generates mutated copies of the RTL from a golden source and grades the testbench on how many it catches. Sites are located with [pyslang](https://pypi.org/project/pyslang/); the number and classes of mutation are set with `--count` and `--types` (`arith`, `bitwise`, `compare`, `logical`, `shift`, `unary`, `pipeline`, `sign`, `array_packed`, `array_unpacked`, `width`, `condition`, `statement`, `operand`, `blocking` and `nonblocking` - the last ten giving a registered signal an extra cycle of delay, negating the value assigned to a signed variable, turning a declared packed or unpacked dimension around, taking a bit off a packed declaration with the references cast so that the mutant still compiles without width warnings, holding a branch permanently open and permanently shut and nudging the comparisons it is built from, dropping a registered assignment so its register holds, turning a non-commutative operator's operands around, and swapping blocking for nonblocking assignment and back where the target is read again in the same block). Every line a mutation changes carries a `/* AVL MUTATION - <what was done> */` comment, so a mutant that turns up in an editor, a debugger or a waveform cannot be mistaken for the golden source. Generation writes `mutations.html`, a report explaining each mutation in its source context. Where [yosys](https://yosyshq.net/yosys/) is installed each mutation is also proved to change the design, so a mutant that no testbench could ever catch is reported as equivalent rather than counted as a verification hole. The campaign leans on the pinned `COCOTB_RANDOM_SEED` below so that the golden design and every mutant see the same stimulus; without it cocotb draws a fresh seed per simulation and a mutation's fate depends on which random numbers its own run happened to draw. `make mutation_regression` runs the testbench against the golden design and every mutant, reports what was caught, keeps each run's log and results, and exits non-zero if any mutation survives. Nineteen examples under `examples/mutation_testing`: sixteen synthetic ones, one per mutation class, each a small DUT holding sites of that class alone with a testbench that detects them, an `equivalent` example whose RTL adds zero, so that one mutation is provably no change at all and the equivalence check is tested rather than assumed, plus a registered ALU split across three files and two sub-modules showing mutation of a hierarchical design, and a single file synchronous FIFO. `make regression` in `examples/mutation_testing` runs every campaign and reports one summary, failing with a non-zero exit status if any example did not run, did not locate every mutation it asked for, or left a mutation undetected; an example that declares a tool it cannot do without, as `equivalent` declares yosys, is skipped rather than failed when that tool is missing. The regression takes `-j`, running that many examples at once - each example has its own working directory, `mutants/` and `sim_build/` - while the campaign inside any one example stays serial, since its mutants share a directory and AVL writes `coverage.json` to the working directory under a fixed name.

### Changed
 - Mutation generation: when there are more candidate sites than `--count`, every class is now represented before any class repeats, and every operator within a class before any operator repeats. Previously the first N in source order were taken, which could spend the whole budget on one class. Classes are prioritised in the order given to `--types`.
 - `avl.sh`: warns when Yosys is not on PATH, in the same way it already does for Graphviz, since without it the mutation testing equivalence stage is skipped.
 - `examples/sim.mk`: every example now pins `COCOTB_RANDOM_SEED ?= 1`, so a run is the same run every time. Previously cocotb drew a fresh seed from the clock per simulation, and an example whose stimulus only occasionally reaches a corner passed or failed on the draw - `sequences/sequence_of_sequences` failed roughly one run in four. Override it to sweep, for one example or for a whole regression: `make sim COCOTB_RANDOM_SEED=42`.
 - `examples/sim.mk`: the RTL to compile is now `AVL_RTL`, defaulting to `$(CURDIR)/rtl/example_hdl.sv` as before, so an example can build a generated copy of its RTL without departing from the standard flow.
 ...
 - [#100](https://github.com/projectapheleia/avl/pull/100) Documentation: the Z3 chapter's link to the Z3 Python API pointed at a page that no longer exists, and a couple of typos alongside it.
