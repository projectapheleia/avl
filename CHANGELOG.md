...
### Added
 ...
 - Benchmark reports: a run is reported whole and one group at a time - `results/report.html` for everything that ran, and `results/<group>/report.html` for each of `randomization`, `create_and_send` and `randomize_and_send` - because a group is a kind of benchmark and its report can be written in the terms of what it measures, where a report spanning several kinds can only be neutral about them and comparing one group against another is meaningless anyway. Each holds the same four files (`report.html`, `summary.csv`, `summary.md`, `quality_summary.csv`) and links to the others by relative path, so `results/` can be copied or published as it stands; the per group reports are skipped when only one group ran, as that would be the combined report again under another name. Every report also records what produced the figures - the processor, its cores and clock, the memory, the operating system and kernel, the version string the simulator reports for itself, and the installed versions of `avl`, `cocotb`, `pyuvm` and `pyvsc` - because a time means little without them, and less still compared against a time from a different machine. A report taken over an incomplete run names the benchmarks it has no results for, rather than looking exactly like a complete one.
 ...
 - `pyuvm` benchmark flavour: every benchmark is measured a third way, as a pyuvm test randomizing a pyvsc `randobj`, so the comparison covers the alternative a Python testbench actually has and not SystemVerilog alone. It needs a testbench of its own, `cocotb/<benchmark>_pyuvm.py`, because the loop has to live in the `run_phase` of a `uvm_test`, but it is built and run by the same makefiles from the same RTL as the other two flavours and drives the same clock, the same reset, the same number of edges and the same checks. `pyuvm` and `pyvsc` are development dependencies of AVL, so `avl.sh` installs them alongside everything else.
 - `benchmarks/`: a suite for comparing AVL against the alternatives and recording how long each one takes, built and run through cocotb's makefiles exactly as the examples are, so a change of `SIM` changes it for every flavour and the comparison stays honest. `benchmarks/randomization` is its first group - `arithmetic`, `bitwise`, `list`, `implication` and `cross`, one per category of constraint, plus `mixed`, the widest spread of them on a bus-transaction shaped item. Outside `implication`, `cross` and `mixed` every constraint controls a single variable, so a category measures its own construct and nothing else. Every flavour of a benchmark compiles the **same RTL** and runs the **same harness**, which generates clock and reset and does one unit of work per rising edge; who does that work is the only thing behind an `` `ifdef `` in the RTL, and the only branch in the testbench. What each measurement is taken against is a baseline run of the same testbench driving the same number of clock cycles with the work under test switched off, so simulator and interpreter startup, elaboration, cocotb bringup and the loop itself are subtracted and what is left is the work alone. Each testbench is run `REPEATS` times and the median taken; `SEED` and `SIM` are applied identically to every flavour, `ITERATIONS` and `BURST` likewise unless a benchmark's `bench.conf` gives one flavour its own, and `make bench BENCHES=...` restricts a run, the report it writes and the clean afterwards to one group or a named few benchmarks. Speed is not reported on its own: a randomizing benchmark also records every value it drew, and the report scores their spread as normalised Shannon entropy and worst-bit skew, so that no flavour can look fast by randomizing badly - `QUALITY_CHECK` fails a run whose spread falls too far behind the others.
