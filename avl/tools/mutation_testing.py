...
Most defects are a single swapped operator. Two are not. ``pipeline`` gives a
registered signal an extra stage so that it arrives one cycle late - a timing
defect rather than a logical one, and the kind a scoreboard that is loose about
when it compares will happily miss. ``sign`` negates the value assigned to a
signed variable, modelling a sign that was dropped or applied the wrong way
round.
...
def references(tree, src: str) -> dict[tuple[str, str], dict]:
    ...
    :return: Per module and name, the spans that read it, the right hand sides
             that are assigned to it, and whether any reference is selected.
    ...
    def entry(node, name):
        return found.setdefault(
            (_enclosing_module(node), name), {"reads": [], "writes": [], "selected": False}
        )
    ...
    def visit(node):
        ...
        # the whole signal is put out of reach.
        if kind == "IdentifierSelectName":
            span = node.sourceRange
            name = src[span.start.offset : span.end.offset].split("[")[0].strip()
            entry(node, name)["selected"] = True
            return
        ...
        span = node.sourceRange
        name = src[span.start.offset : span.end.offset].strip()
        record = entry(node, name)

        parent = node.parent
        if _kind(parent) in SIGN_KINDS and parent.left is node:
            rhs = parent.right.sourceRange
...
def find_sites(path: str, types: list[str]) -> tuple[str, list[dict]]:
    ...
    refs = references(tree, src) if WIDTH in types else {}
    ...
    def assignment_site(node, group, replacement):
        ...
        rest = src[statement.sourceRange.end.offset : scope.sourceRange.end.offset]
        if not re.search(rf"\b{re.escape(target)}\b", rest):
            return None
    ...
    def width_site(node):
        ...
        if record is None or record["selected"]:
            return None
...
def main():
    ...
    if args.list_types:
        ...
        print("    declarations with literal bounds and no bit selects only.")
        print(f"{SIGN}:")
