...
|---|---|---|---|
| `c_a` | list | `a inside {[100:200], [1000:1100], 5000};` | `Or([And(UGE(a, lo), ULE(a, hi)) ...] + [a == 5000])` |
| `c_b` | arithmetic, on `a` | `b > a; b < a + 1000;` | `And(UGT(b, a), ULT(b, a + 1000))` |
| `c_c` | arithmetic, on `b` | `c > b;` | `UGT(c, b)` |
| `c_mask` | bitwise, on `a` and `b` | `(c & 32'h0000_00ff) == ((a ^ b) & 32'h0000_00ff);` | `(c & 0x00FF) == ((a ^ b) & 0x00FF)` |
| `c_shift` | bitwise and arithmetic, on `a` | `(c >> 8) == ((a >> 8) + 1);` | `LShR(c, 8) == LShR(a, 8) + 1` |
| `c_d` | list, on `a`, `b` and `c` | `d inside {a, b, c};` | `Or(d == a, d == b, d == c)` |
