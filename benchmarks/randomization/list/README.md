...
|---|---|---|---|
| `c_a` | a set of sixteen values | `a inside {1, 2, 4, ... 32768};` | `Or([a == v for v in VALUES])` |
| `c_b` | a set of ranges | `b inside {[100:200], [500:600], [5000:5100]};` | `Or([And(UGE(b, lo), ULE(b, hi)) for lo, hi in RANGES])` |
| `c_c` | exclusion | `!(c inside {[0:1000]});` | `Not(And(UGE(c, 0), ULE(c, 1000)))` |
| `c_d` | values mixed with a range | `d inside {7, [20:25], 9999};` | `Or(d == 7, And(UGE(d, 20), ULE(d, 25)), d == 9999)` |
