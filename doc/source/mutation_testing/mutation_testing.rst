...
using `mutation testing <https://en.wikipedia.org/wiki/Mutation_testing>`_.
...
to your path when AVL is installed. It uses
`pyslang <https://pypi.org/project/pyslang/>`_, the Python binding to the
`slang <https://sv-lang.com/>`_ SystemVerilog front end, to locate mutation
sites in the source. pyslang is included in the AVL dependencies, so no separate
installation is needed. Sites are found in a parsed syntax tree rather than in
text, so a mutation is only ever applied to a real operator, declaration or
assignment, and never to something that merely looks like one inside a comment
or a string. The parser is used only to find the exact source span of each site;
the rewrite is a splice over that span, so the mutant keeps the formatting and
comments of the golden source and compiles on any simulator that accepts it.
Running the tests also requires a simulator and the normal testbench build
environment.

`yosys <https://yosyshq.net/yosys/>`_, an open source synthesis and formal
verification tool, is optional and used only when found on your path. The tool
asks it whether each mutant can be told apart from the golden design at all. A
mutant that cannot is dead weight: no testbench could ever catch it, so it is
reported as equivalent rather than counted as a hole in the verification.
Without yosys this stage is skipped and a surviving mutant cannot be told apart
from one that is impossible to detect. See `Equivalence Checking`_ for the
verdicts and the bounds the check works within.
...
Every AVL example pins ``COCOTB_RANDOM_SEED=1``, in ``examples/sim.mk``, so that
a run is the same run every time. A campaign depends on it more than most: the
golden design and all mutants have to receive the same stimulus for a verdict to
mean anything. To use another seed, apply it to the whole campaign:
...
.. list-table::
   ...
   * - ``width``
     - Narrows an internal packed signal by one bit and inserts casts at its
       ...
       Offered only where every reference can carry a cast, which rules out
       bit selects, port connections and assigned-to concatenations.
   * - ``condition``
...
.. list-table::
   ...
   * - ``--output DIRECTORY``
     - Mutant output directory. Default: ``mutants``. Replaced on generation,
       so a directory that the tool did not write itself is refused rather
       than deleted.
   * - ``--types CLASSES``
