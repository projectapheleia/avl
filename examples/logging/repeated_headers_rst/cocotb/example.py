...
# every flush after the first appends another table header. In reStructuredText
# the result is a run of separate simple tables rather than the one table the
# log is meant to be, each sized to its own chunk.
#
...
_LOGFILE_ = "avl_log.rst"
_FLUSH_LEVEL_ = 10
_MESSAGES_ = 10 * _FLUSH_LEVEL_

_HEADER_ = re.compile(r"^ *Time +Level +Group +Message +Filename +LineNo *$", re.MULTILINE)
"""The column-name row of a simple table. One per table."""
