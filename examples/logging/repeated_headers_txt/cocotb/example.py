...
# every flush after the first appends another table header. In the grid text
# format the result is a run of separate tables rather than the one table the
# log is meant to be, each sized to its own chunk, so the columns do not even
...
_LOGFILE_ = "avl_log.txt"
_FLUSH_LEVEL_ = 10
_MESSAGES_ = 10 * _FLUSH_LEVEL_

_HEADER_RULE_ = re.compile(r"\+=+(?:\+=+)+\+")
"""The +====+====+ rule tabulate's grid format puts under a header. One per table."""
...
async def test(dut):
    ...
    headers = _HEADER_RULE_.findall(log)
    assert len(headers) == 1, (
        f"{_LOGFILE_} contains {len(headers)} table headers, expected 1 - "
