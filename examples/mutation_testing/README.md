...
```sh
make regression      # every example, then a summary
make -j 6 regression # six examples at a time
make list            # the examples that would be run
make                 # the targets in this directory, and in any one example
make clean
```
...
---------------------------------------------------------------
  ...
  ...
  equivalent       SKIP  needs yosys
  ...
---------------------------------------------------------------
  18/18 passed, 1 skipped
```

`-j` runs that many examples at once. The campaign inside any one of them stays
serial however many jobs are asked for, so each job costs one simulation's worth
of memory. Results are printed as each example finishes, so under `-j` they
arrive in whatever order the examples complete; the summary does not. A failing
example does not stop the others, and one failure anywhere makes the exit status
non zero.
...
zero, and each run's full log is kept under `.results/<example>.log`. An example
that declares a tool it cannot do without is skipped rather than failed when
that tool is missing, which is how `equivalent` behaves without yosys.
...
The campaign within an example is deliberately serial. Every run shares the
example directory and AVL writes `coverage.json` to the working directory under
a fixed name, so running mutants concurrently here would have them overwrite
each other's coverage. Whole examples are a different matter - each has its own
`mutants/` and `sim_build/`, and each runs with its own directory as the working
directory - so `make -j N regression` above runs them side by side.
...
A reference also has to be somewhere a cast can go, which rules out two places
that look like reads but are really where the signal is driven: a module port
connection, and an element of a concatenation being assigned to. Casting either
one produces RTL that does not build — Verilator calls `.y(9'(sig))` an
electrical short — and a mutant that does not build scores as detected without a
testbench having noticed anything, which is worse than not offering the site.
...
Every AVL example pins the seed, in `examples/sim.mk`:
...
A campaign leans on that harder than most examples do. Without it cocotb draws a
fresh seed from the clock for each simulation, so the golden design and every
mutant see different stimulus. A campaign run that way
...
Sixteen of them are synthetic, one per mutation class. Each is a tiny DUT
holding sites of that class and nothing else, with a testbench small enough to
read in one sitting, so a class can be seen on its own:
...
| `operand` | `y <= {(a < b), (a - b)}` | both non-commutative operators reversed |
| `statement` | `valid_out <= valid_in; y <= {1'b0, a ^ b}` | either registered assignment dropped, so its register holds |
| `blocking` | two-stage `valid_d`/`data_d` pipeline | both `<=` made blocking, where the target is read again below |
| `nonblocking` | `sum = a + b; dif = a - b` in a clocked block | both `=` made nonblocking |
| `array_packed` | `rev[i] = a[7-i] ^ b[i]` over `logic [7:0]` | the four packed ranges — `a`, `b`, `y`, `rev` — turned around |
| `array_unpacked` | `logic [7:0] lut [0:3]` | the unpacked range turned around, reordering the entries |
| `width` | `logic [8:0] total = a + b` | `total` narrowed to `[7:0]`, losing the carry |
| `condition` | six comparisons, three as `if` predicates and three as `?:` | each held open, held shut, and nudged to its neighbouring comparison — 18 in all |

All sixteen share one interface — `clk`, `rst_n`, `valid_in`, `a`, `b` in and
...
Four need stimulus shaped for the class, which is the point of those classes:
...
* `logical` — `&&` and `||` agree unless the two conditions disagree, which
  ...
  the items.
* `condition` — holding a branch constant shows on any input, but nudging `>`
  to `>=` needs the same `a == b` boundary that `compare` does, so the sequence
  lands on it rather than waiting to stumble across it.
* `blocking` — a valid path that is high every cycle cannot show a defect that
  slides `valid_out` by one, so the sequence leaves an idle cycle between items.

The other twelve are exposed by uniform random stimulus. All sixteen score
100%, and yosys proves every one of their mutations differs from its golden
design.

The remaining three examples are less synthetic:

| Example | Design | `--types` | Mutations |
| --- | --- | --- | --- |
| `equivalent` | `pass_through = a + 8'd0`, then `y <= pass_through ^ b` | `arith,bitwise` | the add of zero, which no input can expose, and the xor, which any can |
...
`equivalent` is the one example that needs yosys, and the regression skips it
when yosys is missing rather than failing it. Its RTL adds zero, so flipping
that `+` to a `-` changes nothing a testbench could ever see. Scoring it as a
survivor would report a verification hole that is not there; the equivalence
check proves it is no change at all and leaves it out of the score, which is
what makes the claim below about the other two worth anything.

`alu` and `fifo` both score 100%, and yosys proves every mutation differs from
its golden design, so those are real detections rather than mutants that could
not have failed.
