...
#
# Description:
# AVL mutation testing example - blocking
#
...
def model(a, b):
    ...
    a exclusive ored with b. The DUT takes two cycles to say so.
...
class mutation_sequence(avl.Sequence):
    ...
    async def body(self):
        ...
        for _ in range(self.n_items):
            ...
            item.randomize()
            # Uniform random stimulus is enough to expose this class.
...
class mutation_driver(avl.Driver):
    ...
    async def run_phase(self):
        ...
        while True:
            ...
            # An idle cycle between items. Without it valid_in is high on every
            # cycle, valid_out is high on every cycle too, and sliding valid_out
            # one cycle either way cannot be seen - which is exactly the hole
            # the blocking mutation on the valid path walks through.
            await RisingEdge(self.hdl.clk)
            await self.reset()
...
class mutation_monitor(avl.Monitor):
    ...
    async def collect_result(self, item):
        # Two cycles after valid_in the answer is on the output - the DUT is a
        # two stage pipeline, and it is the handover between those stages that
        # this example mutates. Collapse a stage and the answer turns up here a
        # cycle early, which is what the scoreboard sees.
        await RisingEdge(self.hdl.clk)
        await RisingEdge(self.hdl.clk)
