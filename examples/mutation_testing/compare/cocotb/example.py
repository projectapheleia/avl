...
#
# Description:
# AVL mutation testing example - compare
#
...
def model(a, b):
    ...
    :rtype: int
    """
    return ((1 if a < b else 0) << 1) | (1 if a == b else 0)
...
class mutation_sequence(avl.Sequence):
    ...
    async def body(self):
        ...
        for i in range(self.n_items):
            ...
            item.randomize()
            # < and <= differ only when a == b, so the stimulus has to
            # land on that boundary rather than hope to stumble across it.
            if i % 4 == 0:
