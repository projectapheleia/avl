...
#
# Description:
# AVL mutation testing example - logical
#
...
def model(a, b):
    ...
    Bit 1 is a>0 and b>0, bit 0 is a>0 or b>0.
    ...
    :rtype: int
    """
    both = 1 if (a > 0 and b > 0) else 0
    either = 1 if (a > 0 or b > 0) else 0
    return (both << 1) | either
...
class mutation_sequence(avl.Sequence):
    ...
    async def body(self):
        ...
        for i in range(self.n_items):
            ...
            item.randomize()
            # && and || agree unless the two conditions disagree, which
            # needs one of the operands to be zero.
            if i % 4 == 0:
                item.a.value = 0
            elif i % 4 == 1:
                item.b.value = 0
