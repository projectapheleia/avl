...
# The seed is pinned for every example by ../../sim.mk below, and a campaign
# depends on it more than most: the golden design and each mutant have to see
# the same stimulus, or whether a mutation was caught comes down to which random
# numbers its own run happened to draw rather than to whether the testbench can
# catch it. Boundary mutations suffer worst - they need a particular input, so
# they are exactly the ones a drifting seed decides by luck. Override it to
# sweep, but sweep the whole campaign together.
