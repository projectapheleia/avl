...
class example_env(avl.Env):
    ...
    def test_nested_structs(self):
        ...
        print(f"init_val = {hex(init_val)}")
        #using the from_bits function
        full_struct.from_bits(init_val)
        print(f"Calling check_struct for inital value")
        self.check_struct(full_struct, expected)
        #using the to_bits function
        print(f"Setting a varible using to_bits function")
        to_bits_var = full_struct.to_bits()
        print(f"Value set is equal to {hex(to_bits_var)}")
